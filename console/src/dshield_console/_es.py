...
`src/dshield_enrich/es_client.py`. Kept separate so the console has no
...
def make_client(cfg: ESConfig, secrets: Secrets) -> Elasticsearch:
    ...
    # one-shot `SecurityWarning`. See src/dshield_enrich/es_client.py for
    # the rationale; the two helpers are intentionally duplicated so the
    # console package has no Python dependency on dshield_enrich.
    ...
    if secrets.es_api_key:
        ...
        raise RuntimeError(
            ...
            "searched in this order: $DSHIELD_CONSOLE_ENV, "
            "$DSHIELD_ENRICH_ENV, alongside-config-file's parent, "
            "alongside-config-file, CWD."
        )
