...
| `cluster ips` | Phase 4b: HDBSCAN over IP embeddings (augmented with IP-level scalars). The scalar matrix has two sub-blocks: **behavior** (`total_sessions`, `login_success_rate`, `mean_novelty_score`, `mean_session_duration_s` at `ip.cluster_scalar_weight=0.05`) and **attribution** (country one-hot + top-N ASN bucket + credential hash at `ip.cluster_attribution_weight=0.10`). Attribution keeps two-bots-running-Mirai-from-different-ASNs from clustering together. Writes `dshield.cowrie.enrichment.ip.cluster.*` to each IP doc and centroid docs to the IP-clusters index. Requires `.[cluster]` extras. ROADMAP issue #8. |
...
| `name playbooks` | Two-pass naming for non-outlier session clusters (each is a **playbook**). **Pass 1**: each cluster gets a name via the local LLM, no awareness of other clusters. **Pass 2** (ROADMAP #10): if any pass-1 names collide — either within this run or against playbooks named in any prior run — the LLM is re-prompted per collision group with all colliders side-by-side and asked to produce distinct names that capture what's substantively different (IOC, intent, tooling, behavioural signature). Output looks like `curl Pipe Bash Dropper via 47.83.203.183 C2` vs `curl Pipe Bash Dropper via 43.161.230.246 C2` rather than a bare suffix. Pass 2 soft-fails: if the LLM is unreachable, returns unparseable JSON, or produces still-colliding new names, the pass-1 names are kept. Set `prompts.playbook_disambiguate: null` in config to disable pass 2 entirely. Writes `playbook_id` + `playbook_name` to the session-clusters centroid and back-fills both onto every member session doc. |
...
| `novel_embedding` | Phase 3: fired by the `escalate` command (not `enrich`) — queries ES for locally-enriched docs whose stored `novelty_score` ≥ `cloud.triage.novel_embedding_threshold` and whose `confidence` is in `[novel_confidence_min, escalate_confidence_max]`, then re-escalates them. The lower confidence bound filters out encoding-artifact docs whose novelty signal is meaningless (raw bytes etc.). During `enrich`, embedding is always the last step (after all LLM calls), so novelty distance is not computed at enrich time |
...
`cluster ips` clusters the IP embeddings augmented with two scalar sub-blocks: behavior signals (`total_sessions`, `login_success_rate`, `mean_novelty_score`, `mean_session_duration_s`) at `ip.cluster_scalar_weight=0.05`, and attribution signals (`country` one-hot, top-N `ASN` bucket, `credential` hash) at `ip.cluster_attribution_weight=0.10`. Attribution is intentionally hotter — without it, two unrelated bots running identical Mirai variants from different ASNs would cluster together. IP clusters are unnamed "actor profile" buckets — they're not LLM-named. An IP's playbook membership is derived from the sessions it produced; an IP's campaign membership is derived from `mine campaigns` (which writes to its own index, not back onto IP docs).
...
| `dshield.cowrie.enrichment.ip.credentials` | keyword[] | Sorted dedup'd `user:pass` tuples observed across this IP's sessions (capped at 200). Hashed into the attribution scalar block by `cluster ips` (ROADMAP #8). |
